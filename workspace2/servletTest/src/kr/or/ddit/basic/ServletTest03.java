...
/*
    서블릿의 동작 방식에 대하여...
    1. 사용자(클라이언트)가 URL을 클릭하면 HttpRequest를 Servlet Containner로 전송(요청)한다.
    2. 컨테이너는 web.xml에 정의된 url패턴을 확인하여 어느 서블릿을 통해서 처리할것인지를 검색한다.
       ( 이 때 로딩이 안된 경우에는 해당 서블릿을 로딩한다. 최초에 로딩할 때 init()메서드를 자동으로 호출한다.)
       (Servlet 3.0dltkddptjsms doshxpdltus(@webServlet)으로 설정 가능)
    3. Servlet Containner는 요청을 처리할 개별 쓰레드 객체를 생성하여 해당 서블릿 객체의 service()메서드를 호출한다.
       ( 이 때 HttpRequest 및 HttpResponse객체를 생성하여 파라미터로 넘겨준다.)
    4. service()메서드는 요청의 메서드 타입(get 또는 post)을 체크하여 적절한 메서드를 호출한다.
       (doGet(), doPost(), doPut(), doDelete() 등)
    5. 요청 처리가 완료되면 HttpRequest 및 HttpResponse객체는 자동으로 소멸된다.
...
public class ServletTest03 extends HttpServlet {
   ...
   protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      // 방법2 ==> 클라이언트 요청 방식(GET, POST 등)을 구분해서 직접 메서드를 호출하기

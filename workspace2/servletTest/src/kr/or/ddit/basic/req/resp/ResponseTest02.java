...
public class ResponseTest02 extends HttpServlet {
	...
	/*
	 ...
	 2) 응답시 브라우저에게 '이동할URL'을 전송하면 브라우저가 해당 URL로 이동 재용청하는 방식이다.
	 3) 재요청할 때는 GET방식으로 요청하기 때문에 URL의 길이에 제한이 있다.
	 */
		// redirect는 Request객체를 유지하지 못한다. => 이유 : 브라우저에서 새롭게 요청하기 때문에..
...
//		response.sendRedirect(request.getContextPath() + "/responseRedirectTest.do");
		...
		response.sendRedirect(request.getContextPath() + "/responseRedirectTest.do?username=" + name + 
								"&tel=" + tel);
	}

...
public class ServletTest04 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/*
		 ...
		 - getMajorVersion()과 getMinorVersion()메서드는 웹 컨테이너가 지원하는
		   Servlet 규격서의 메이져 버전과 마이너 버전을 구할 수 있다
		 */
		...
		out.printf("지원하는 Servlet 버전 : (%d.%d)<br><br>", majorVer, minorVer);
		...
		/*
		 ...
		 - web.xml문서의 <web-app>엘리먼터 바로 아래에 <context-param>이라는
		   서브 엘리먼트를 작성하고, 그 안에<param-name>과 <param-value>라는 서브 엘리먼트 값들을 설정한다.
		   
		 - 웹 애플리케이션의 초기화 파라미터를 읽기 위해서는 
		   1) servlet클래스는 getSercletContext()메서드를 통해서 ServletContext객체를 구하고, 
		             이 객체의 getInitParameter()메서드로 값을 읽어와 사용할 수 있다.
		 	
		   2) JSP페이지에서는 application 내장 변수의 getInitParameter()메서드를 사용하면된다.
		 */

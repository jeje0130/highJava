...
/*
 	servlet 3.0이상에서는 파일 업로드를 하려면 @multipartConfig 애노테이션을 설정해야한다.
 	- @multipartConfig 애노테이션에서 설정할 변수들
 		...
 		3. maxRequestSize : 서버로 전송되는 request이 최대 크기 (단위 : byte, 기본값 : -1L(무제한))
...
public class FileUpLoadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//파일이 아닌 데이터들을 getParameter()메서드나 getParameterValues()메서드를 
		// 이용하여 구한다.
		...
		// 업로드한 파일의 정보가 저장될 List 객체 생성
		List<UploadDetail> fileList = new ArrayList<UploadDetail>();
		
		
		/*
		 	- Servlet 3.0 이사에 새롭게 추가된 파일 Upload용 메서드
		...
		for(Part part : request.getParts()) {
			...
			if(!"".equals(fileName)) {
				...
				fileList.add(detail); //파일정보를 저장한 객체를 List에 추가
			}

...
//		p2.setName("일지매");
		...
		System.out.println(p1==p2);       // 참조값은 다름
		System.out.println(p1.equals(p2)); // equals로 비교해서 true가 나옴
		System.out.println("----------------------------");
		 ...
		 *   컬렉션 객체들을 객체의 의상의 동일성을 비교하기 위해서 hashCode()메서드를
		 *   호출해서 비교한다. 그러므로 객체가 같은지 여부를 결정하려면
		 *   hashCode를 재정의 해야한다.
		 * 
		 * - hashCode()메서드에서 사용하는 '해싱 알고리즘'은 서로 다른 객체들에 대하여 같은
		 * 	 hashcode값이 나타날 수 있다.
		 */

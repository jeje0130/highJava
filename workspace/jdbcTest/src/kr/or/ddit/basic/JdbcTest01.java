...
public class JdbcTest01 {
	...
	/*
	 ...
	 4. 결과처리 ==> 질의 결과를 받아서 원하는 작업을 수행한다
	 	...
	 	2) SQL문이 select문이 아닐 경우(insert, update, delete, create 등...)에는 
	 	      정수값이 반환된다. (이 정수값은 보통 해당 명령이 실행에 성공한 레코드 수가 된다.)
	 
	 5. 사용한 자원 반납하기 ==> close()메서드 사용
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 2. DB 연결 => 연결 성공시 Connection객체 생성
			conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "YJH01", "java");
			
			//프로젝트DB
			/*String url = "jdbc:oracle:thin:@dditbasic.csqutft2bzky.ap-northeast-2.rds.amazonaws.com:1521:ORCL";
			String user = "admin";
			String password = "ddit302java";
			...
			// rs.next() ==> ResultSet객체의 데이터를 가리키는 포인터를 다음번째 레코드로
			//				이동시키고 그 곳에 데이터가 있으면 true를 반환한다.
			while(rs.next()){
